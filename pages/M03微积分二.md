...
<div align="right">最近更新时间：2023-09-08</div>
...
&若g(x,y,z)仅包含z，则可以用面积公式直接求解\\
=& \int_{z下限}^{z上限}g(x,y,z)dz\iint_D1dxdy\\
=& S_D\int_{z下限}^{z上限}g(x,y,z)dz\\
\end{align*}
$$
...
**例1：柱面坐标，圆柱面**：$\Omega$为：$x^2＋y^2＝z，z≤4$，求$\iiint_{\Omega} g(x,y,z)dxdydz$
...
**例2：柱面坐标，椭圆柱面**：$\Omega$为：$\frac{x^2}{a^2}＋\frac{y^2}{b^2}＝z，z≤4$，求$\iiint_{\Omega} g(x,y,z)dxdydz$
...
| 收敛的必要不充分条件是：通项趋于0，有$\lim_{n\to\infty}通项_n=\lim_{n\to\infty}通项_{n+1}$ |
| 收敛通项＋发散通项，必发散                                                        |
| 发散通项＋发散通项，未必发散                                                       |
| 若\|通项\|收敛，则原通项收敛                                                     |
...
        用三元方程求点的**法向量**：$(f_x'，f_y'，f_z')$

        用三元方程求过**切点**$(x_0,y_0)$**切平面**：$f_x'(x-x_0)＋f_y'(y-y_0)＋f_z'(z-z_0)＝0$

        用三元方程求过**切点**$(x_0,y_0)$**法线**：$\frac{x-x_0}{f_x'}=\frac{y-y_0}{f_y'}=\frac{z-z_0}{f_z'}$
...
向量$A(x,y,z)=Pi+Qj+Rk$，其中$P、Q、R$为函数可偏导，则有$A$的**旋度**为
